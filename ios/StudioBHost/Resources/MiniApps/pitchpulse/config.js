const appConfig = {
  ...
  plan: {
    derive: ({ values }) => {
      const beats = [
        `Hook for ${values.audience || "listener"} with ${values.time || "30"}s.`,
        `Proof: ${values.focus || "traction"} angle.`,
        `CTA tailored to time window.`
      ];
      return { hook1: beats[0], hook2: beats[1], hook3: beats[2] };
    },
    summary: {
