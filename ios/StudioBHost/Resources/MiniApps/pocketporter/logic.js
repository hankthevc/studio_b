...
export async function planPackingList(formValues = {}) {
  ...
  let llmSeed = null;
  try {
    llmSeed = await callPocketPorterLLM({
    ...
    console.warn("PocketPorter LLM failed, using local generator.", error);
  }

  const categories = normalizeCategories(llmSeed?.categories) || buildCategories(vibe, climate);
  const compliance = normalizeCompliance(llmSeed?.compliance, form.airline, categories) || buildCompliance(form.airline, categories);
  ...
  return {
    ...
    categories,
    compliance,
    weatherTip: llmSeed?.weatherTip || climateTips[climate],
...
function normalizeCategories(raw) {
  ...
  return raw.map((category) => {
    const items = Array.isArray(category?.items)
      ...
      : [];
    const sortedItems = items.sort((a, b) => Number(b.required) - Number(a.required));
    return {
      name: category?.name || "Category",
      badge: category?.badge || "Optional",
      items: sortedItems
    };
  });
}

function normalizeCompliance(compliance, airline, categories) {
  if (!compliance) return null;
  return {
    status: compliance.status || "clear",
    message: compliance.message || buildCompliance(airline, categories).message,
    allowanceKg: compliance.allowanceKg || 10,
    liquidsMl: compliance.liquidsMl || 100
  };
}
import { callPocketPorterLLM } from "../../../shared/llmClient.js";
