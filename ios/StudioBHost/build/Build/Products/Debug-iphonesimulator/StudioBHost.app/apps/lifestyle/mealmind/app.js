...
export async function initMiniApp(container) {
  ...
  try {
    const isSubscribed = await window.MiniHost.isSubscribed("mealmind");
...
function buildUpsellBanner() {
  ...
  button.addEventListener("click", async () => {
    ...
    try {
      const result = await window.MiniHost.requestSubscription("mealmind");
      if (result.status === "active") {
        state.isSubscribed = true;
        card.classList.add("is-hidden");
        showToast("Welcome to MealMind Pro!");
      ...
      button.textContent = "Upgrade to MealMind Pro";
    }
  });
