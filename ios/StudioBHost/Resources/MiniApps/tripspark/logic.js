...
function buildItineraryFromSeed(form, llmSeed) {
  ...
  const normalizedSeed = normalizeSeed(llmSeed, form.vibe);
  const daySeeds = normalizedSeed.days;
  const highlight = normalizedSeed.sparkHighlight;
...
function normalizeSeed(seed, vibe) {
  const idea = vibeIdeas[vibe];
  const safeHighlight =
    typeof seed?.sparkHighlight === "string" && seed.sparkHighlight.trim().length > 0
      ? seed.sparkHighlight.trim()
      : idea.highlight;
  const rawDays = Array.isArray(seed?.days) ? seed.days : [];
  const days = [0, 1, 2].map((idx) => {
    const daySeed = rawDays[idx] || {};
    return {
      day: idx + 1,
      theme: daySeed.theme || `Signature day ${idx + 1}`,
      highlights: Array.isArray(daySeed.highlights) && daySeed.highlights.length
        ? daySeed.highlights.slice(0, 3)
        : idea.segments
    };
  });
  return { sparkHighlight: safeHighlight, days };
}
