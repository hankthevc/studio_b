...
export async function initMiniApp(container) {
  ...
  try {
    const isSubscribed = await window.MiniHost.isSubscribed("focustiles");
...
function buildUpsell() {
  ...
  button.addEventListener("click", async () => {
    ...
    try {
      const result = await window.MiniHost.requestSubscription("focustiles");
      if (result.status === "active") {
        state.isSubscribed = true;
        card.classList.add("is-hidden");
        showToast("Welcome to FocusTiles Pro!");
        const resultsSection = document.querySelector(".results");
        if (resultsSection && state.lastPlan) {
          renderTiles(resultsSection, state.lastPlan, card);
        }
      } else {
